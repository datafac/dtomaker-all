...
public class ConverterTests_Enum
{
    ...
    [Fact] public async Task CustomSrcGen_VerifyGeneratedSource2() => await Verifier.Verify(new SourceGenerator().GenerateAndGetOutput(modelSource, 1, "MyOrg.Models.MsgPack2.Domain.g.cs"));
    [Fact] public async Task CustomSrcGen_VerifyGeneratedSource3() => await Verifier.Verify(new SourceGenerator().GenerateAndGetOutput(modelSource, 2, "MyOrg.Models.MsgPack2.MyDTO.g.cs"));
}