...
public sealed class EntityGenerator : EntityGeneratorBase
{
    ...
    protected override void OnGenerate(OutputEntity entity)
    {
        ...
        if (entity.EntityId == 0)
        {
            ...
            Emit("        protected virtual int OnGetEntityId() => T_EntityId_;");
            Emit("        public int GetEntityId() => OnGetEntityId();");
            Emit("");
            Emit("        private static EntityBase CreateEmpty()");
            Emit("        {");
            Emit("            var empty = new EntityBase();");
            Emit("            empty.Freeze();");
            Emit("            return empty;");
            Emit("        }");
            Emit("        private static readonly EntityBase _empty = CreateEmpty();");
            Emit("        public static EntityBase Empty => _empty;");
            Emit("");
            Emit("        public static EntityBase CreateFrom(EntityBase source)");
            Emit("        {");
            ...
            Emit("                _ => new EntityBase(source)");
            Emit("            };");
            Emit("        }");
            Emit("");
            Emit("        public new static EntityBase CreateFrom(IEntityBase source)");
            Emit("        {");
            Emit("            if (source is EntityBase concrete && concrete.IsFrozen) return concrete;");
            Emit("            return source switch");
            Emit("            {");
            foreach (var derived in entity.DerivedEntities.OrderByDescending(e => e.ClassHeight))
            {
            ...
            Emit("        [JsonIgnore] public bool IsFrozen => _frozen;");
            ...
            Emit("            _frozen = true;");
            Emit("        }");
            Emit("        protected virtual IEntityBase OnPartCopy() => new EntityBase(this);");
