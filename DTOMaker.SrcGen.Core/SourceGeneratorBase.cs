...
namespace DTOMaker.SrcGen.Core
{
    ...
    //{
    //    public static StringBuilder AppendLine(this StringBuilder sb, string value)
    //    {
    //        sb.Append(value);
    //        sb.Append('\n');
    //        return sb;
    ...
    public abstract class SourceGeneratorBase : IIncrementalGenerator
    {
        ...
        protected abstract Phase1Entity OnCustomizePhase1Entity(Phase1Entity entity, Location location, IReadOnlyList<OutputMember> members);
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            {
                var sortedEntities = entities.OrderBy(e => e.EntityId).ToImmutableArray();
                // generate global helpers
                {
                    string nameSpace = sortedEntities.FirstOrDefault()?.TFN.Intf.Space ?? "GeneratedNamespace";
                    var sb = new StringBuilder();
                    sb.AppendLine("// <auto-generated/>");
                    sb.AppendLine("// Entities:");
                    foreach (var entity in sortedEntities)
                    {
                        sb.AppendLine($"// - {entity.TFN.Intf.Space}.{entity.TFN.Intf.Name} ({entity.Members.Count} members)");
                    }
                    sb.AppendLine("// End.");
                    spc.AddSource($"{nameSpace}.Global.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
                }
                // generate domain helpers
                {
                    string nameSpace = sortedEntities.FirstOrDefault()?.TFN.Impl.Space ?? "GeneratedNamespace";
                    var sb = new StringBuilder();
                    sb.AppendLine("// <auto-generated/>");
                    sb.AppendLine("// Entities:");
                    foreach (var entity in sortedEntities)
                    {
                        sb.AppendLine($"// - {entity.TFN.Impl.Space}.{entity.TFN.Impl.Name} ({entity.Members.Count} members)");
                    }
                    sb.AppendLine("// End.");
                    string hintName = $"{nameSpace}.Domain.g.cs";
