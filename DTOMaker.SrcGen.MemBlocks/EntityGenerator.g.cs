            ...
            switch (member.Kind)
            {
                ...
                case MemberKind.Entity:
                    if (member.IsNullable)
                    {
                        ...
                        Emit("            {");
                        Emit("                writableField.Span.Clear();");
                        Emit("            }");
                        Emit("            else");
                        Emit("            {");
                        Emit("                await _T_NullableEntityMemberName_.Pack(blobStore, cancellation);");
                        Emit("                ReadOnlyMemory<byte> buffer = await _T_NullableEntityMemberName_.Serialize(cancellation);");
                        Emit("                await PackData(buffer, writableField, blobStore);");
                        Emit("            }");
                        Emit("        }");
                    ...
                    else
                    {
                        ...
                        Emit("            ReadOnlyMemory<byte> buffer = await _T_RequiredEntityMemberName_.Serialize(cancellation);");
                        Emit("            await PackData(buffer, writableField, blobStore);");
                        Emit("        }");
                ...
                case MemberKind.Binary:
                    if (member.IsNullable)
                    {
                        ...
                        Emit("            else");
                        Emit("            {");
                        Emit("                ReadOnlyMemory<byte> buffer = _T_NullableBinaryMemberName_.AsMemory();");
                        Emit("                await PackData(buffer, writableField, blobStore);");
                        Emit("            }");
                        Emit("        }");
                    ...
                    else
                    {
                        ...
                        Emit("            var buffer = _T_RequiredBinaryMemberName_.AsMemory();");
                        Emit("            await PackData(buffer, writableField, blobStore);");
                        Emit("        }");
